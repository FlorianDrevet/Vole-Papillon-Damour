...
| **Prochaine action** | Ouvrir puis faire relire la PR `fix/scan-continuous-workflow`, effectuer le retest authentifié sur iPhone, puis relever les signaux `Sweep`/`Enrich` et préparer `P1-9` à `P1-11`. Le référentiel externe, le compte et l'envoi d'alertes restent P3. |
| **Dernière machine** | Windows — `C:\Users\flori\RiderProjects\Vole-Papillon-Damour-scan-workflow` |
| **Dernière mise à jour** | 2026-09-04 — retours de test Scanette intégrés dans un worktree dédié ; runtime Books déployé avec migrations EF et smoke tests publics validés |
| **Branche** | `fix/scan-continuous-workflow` — PR à ouvrir après validation finale |
