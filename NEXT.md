...
| **Dernière machine** | Windows — `C:\Users\florian.drevet\RiderProjects\Vole-Papillon-Damour-scanette-auth-camera` |
| **Dernière mise à jour** | 2026-09-04 — garde Entra `Tri`, caméra automatique et cadrage mobile du Scan |
| **Branche** | `feat/scanette-auth-camera` — worktree depuis `main`, changements locaux non déployés |
...
La validation locale de cette reprise passe par 60 tests ChromeHeadless, le contrat de
bootstrap MSAL, le build Scan de production et un contrôle navigateur aux largeurs 375 px,
768 px et 1440 px. Le contrôle confirme que l'écran de connexion reste centré sans
défilement ; le parcours caméra réel avec un compte Entra `Tri` et un iPhone reste à
valider sur l'appareil et l'origine HTTPS de déploiement.
