...
| | |
|---|---|
| **Lot en cours** | `P1-4` — socle interne des handlers de scan, session, caisse, métadonnées, suppression, alertes, correction de session, rebond et idempotence fournisseur validés localement ; `QT-02` échoue à être relevée depuis la session Azure disponible ([palier 1](docs/bourse-aux-livres/plan/02-palier-1-socle-interne.md)) |
| **Prochaine action** | Brancher le transport ACS/Event Grid et son handshake sur le traitement idempotent, puis exposer l'API ; demander une session Azure dans le locataire de l'abonnement pour relever `QT-02` ; le worker reste gelé |
