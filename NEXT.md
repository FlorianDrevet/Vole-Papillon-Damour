...
| | |
|---|---|
| **Lot en cours** | `P1-4` — socle interne des handlers de scan, session, caisse, métadonnées, suppression, alertes, correction de session, rebond, idempotence fournisseur et endpoint Event Grid validés localement ; `QT-02` échoue à être relevée depuis la session Azure disponible ([palier 1](docs/bourse-aux-livres/plan/02-palier-1-socle-interne.md)) |
| **Prochaine action** | Passer à `P1-5` : exposer les contrats de scan/watchlist et la synchronisation hors ligne ; demander une session Azure dans le locataire de l'abonnement pour relever `QT-02` ; le worker reste gelé |
