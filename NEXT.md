...
| | |
|---|---|
| **Lot en cours** | `P1-4` — socle interne des handlers de scan, session, caisse, métadonnées, suppression, alertes, correction de session et rebond validé localement ; `QT-02` échoue à être relevée depuis la session Azure disponible ([palier 1](docs/bourse-aux-livres/plan/02-palier-1-socle-interne.md)) |
| **Prochaine action** | Compléter `P1-4` par l'idempotence fournisseur et le transport ACS/Event Grid, puis exposer l'API ; demander une session Azure dans le locataire de l'abonnement pour relever `QT-02` ; le worker reste gelé |
