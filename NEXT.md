...
Le déploiement manuel [Infra - deploy #35846830759](https://github.com/FlorianDrevet/Vole-Papillon-Damour/actions/runs/35846830759), en mode `deploy` sur `main` (`32462e6`), a échoué pendant la validation ARM. Scan et BackOffice ont rejeté `template.scale.cooldownPeriod` parce que le module Container App utilisait encore l'API `2024-03-01`, qui ne déclare pas cette propriété. Les trois modules Failure Anomalies ont reçu `PT6H` (360 minutes) depuis la fréquence des alertes planifiées ; Azure indique que cette fréquence n'est pas prise en charge par `FailureAnomaliesDetector`.

Le correctif en cours met le module Container App à l'API stable `2025-07-01` et laisse les Failure Anomalies à leur fréquence dédiée `PT5M`. Le run ARM était une opération de déploiement réelle, pas un `what-if` ; l'état Azure après son échec n'a pas été vérifié et peut inclure des mises à jour partielles. Avant le prochain `deploy`, relever les opérations/états du déploiement `vpd-infra-65` et lancer un nouveau `what-if`. Le run précédent `35662330783` du 21 septembre était un `what-if` seulement, donc il ne valide pas l'application réelle de ces changements.
...
authentifié, appareil réel, déploiement ou changement Azure n’a été effectué. La [PR #198](https://github.com/FlorianDrevet/Vole-Papillon-Damour/pull/198)
est ouverte vers `main` et n’est pas fusionnée.
...
### État actualisé — 2026-09-15 — recherche Catalog limitée aux disponibles

Depuis `origin/main` fraîchement récupéré dans le worktree
`Vole-Papillon-Damour-catalog-search-available-only`, l'onglet `/recherche` envoie désormais
`availability=available` par défaut et coche « Disponible maintenant » dès son ouverture. La
réinitialisation des filtres revient au même périmètre. `/catalogue` conserve le catalogue
complet par défaut et le filtre explicite « Tout » permet toujours de retrouver les titres
épuisés pour une liste de recherche.

Validation locale : TDD rouge puis vert, 27 tests ciblés, 210 tests Catalog ChromeHeadless, build
Catalog SSR/navigateur, `python -m graphify update .`, `git diff --check` et smoke Chrome desktop
...
### État actualisé — 2026-09-12 — centrage du loader de recherche Catalog

Depuis `origin/main` fraîchement récupéré dans le worktree
`Vole-Papillon-Damour-catalog-search-loader-centered`, le loader de la section « Pas encore
dans la bourse aux livres » est placé dans une zone dédiée centrée horizontalement et
verticalement, avec une hauteur minimale pour ne plus coller à l'en-tête. Le comportement est
...
### État actualisé — 2026-09-15 — inventaire Catalog : pagination API et recherche différée

Depuis `origin/main` fraîchement récupéré dans le worktree
`Vole-Papillon-Damour-inventory-ux`, l'inventaire de l'administration Catalog ne rend plus
les encarts d'aide et d'historique devenus inutiles, ni l'action « Afficher toutes les fiches ».
La liste reste chargée par page via `GET /books/admin/books` : la recherche et la pagination
transmettent leurs paramètres à l'API et le navigateur ne reçoit que la page demandée. La
recherche se lance automatiquement deux secondes après la dernière frappe, en revenant à la
première page ; le bouton « Actualiser » affiche le loader annulaire partagé pendant le
chargement. Les boutons de ligne conservent la correction confirmée d'une unité avec la note
d'audit existante.

Validation locale : TDD rouge puis vert, 209 tests ChromeHeadless Catalog et build
SSR/navigateur passants ; les avertissements de budget Angular existants restent présents.
`graphify update .` et `git diff --check` passent après la dernière mise à jour de la branche.
Aucun déploiement, changement Azure/Entra, compte, donnée ou API distante n'a été effectué ; le
contrôle authentifié avec API joignable reste à faire.
...
Le worktree `Vole-Papillon-Damour-catalog-search-loader-centered` réserve désormais une zone
...
effectué. La PR [#152](https://github.com/FlorianDrevet/Vole-Papillon-Damour/pull/152) est
ouverte pour relecture.
...
### État actualisé — 2026-09-11 — badge administrateur du header Catalog

Depuis `origin/main` dans le worktree `Vole-Papillon-Damour-admin-header-link`, le header
Catalog ne duplique plus l'accès `Espace administrateur` dans la navigation desktop ou le
drawer mobile. Le tag `Administration` devient le lien unique vers `/administration` et
partage la hauteur de contrôle de `42px` avec le bouton de compte, avec des états hover/focus
accessibles.

Validation : test TDD rouge puis vert, 160 tests Catalog ChromeHeadless, build SSR/navigateur
avec l'avertissement de budget initial connu, `graphify update .` et `git diff --check`.
La PR [#141](https://github.com/FlorianDrevet/Vole-Papillon-Damour/pull/141) est ouverte ; aucun
déploiement ni changement Azure, Entra, compte ou API publique n'a été effectué.
...
### État actualisé — 2026-09-11 — sections de l'accueil Catalog

Depuis `origin/main` fraîchement récupéré dans le worktree
`Vole-Papillon-Damour-catalog-home-sections`, la page d'accueil du Catalog conserve son
header actuel mais reprend les blocs utiles de la maquette : « Parcourir — Par genres » et
« Avec un compte — Suivez un livre, on vous prévient quand il arrive. » sont placés après
« Livres rares », et « Votre sélection » est supprimé. Les livres de « Arrivés cette
semaine » utilisent une variante compacte des cartes de la maquette ; les cartes rares
conservent leur rendu existant et le placeholder de couverture reste utilisé sans image.

Validation : tests TDD rouge puis vert, 19 tests ciblés et 128 tests Catalog complets, build
...
deux descriptions vidéo de `/association/photos` sont corrigées : le film « Vole, Papillon
d’amour » est présenté comme le combat de Maxence, et « Le clip » mentionne la chanson et
la participation de Michael Jones. L’entrée du menu, le pied de page, le fil d’Ariane, le
titre de page et les libellés de galerie utilisent désormais « Photos et vidéothèque ».

Validation : 74 tests ChromeHeadless Website, build SSR/prérendu de 25 routes, Graphify et
...
La branche `feat/upcoming-bourses-card` améliore `/prochaines-dates` dans `src/Catalog` :
la carte de la prochaine édition est plus lisible, ses icônes réutilisent les assets du
...
### État actualisé — 2026-09-06 (post-merge et configuration ACS)

`P2/P3` est livré dans `origin/main` au commit `ead9e79` (PR #72 fusionnée). Les runs
`Infra - deploy` `34031915751`, `Books runtime - deploy` `34032046904`, `Website - deploy`
`34032231011`, `BackOffice - deploy` `34032230503`, `Catalog - deploy` `34032230663` et
`Scan - deploy` `34032231264` sont réussis. L'API expose les lectures anonymes du
...
les œuvres, les pages légales, un espace membre `/compte` avec connexion, inscription, lecture et
...
reçu » reste réservé au futur raccordement du référentiel externe ; l'envoi des alertes est
activé dans l'environnement dev et attend seulement un test réel avec une boîte destinataire.
...
Les deux CNAME DKIM sont publiés sous `selector1/2-azurecomm-prod-net._domainkey.mail`.
La ressource Communication Services `vpd-acs-comm-dev` est créée et liée au domaine Email
vérifié. L'identité managée du Worker possède le rôle `Communication and Email Service Owner`
sur cette ressource ; le secret du webhook est dans `vpd-kv-dev`, référencé par l'API. Le
Worker est activé avec l'expéditeur `DoNotReply@mail.volepapillondamour.fr`, et
l'abonnement Event Grid `vpd-acs-email-delivery-reports-dev` envoie
`Microsoft.Communication.EmailDeliveryReportReceived` vers
...
`P1-11` restent à faire. La vérification ACS et le câblage runtime sont terminés ; le cycle
d'e-mail réel reste à exécuter avec un destinataire autorisé.
...
vérification du domaine ACS est terminée pour la propriété, le SPF et les deux sélecteurs
DKIM. La réputation du domaine d'envoi reste à construire et le cycle d'e-mail réel reste
à relever.
...
> |---|---|---|
> | Heartbeat du nouveau `Sweep`/`Enrich` (`P1-6`/`P1-7`) | `2026-09-05`, après le run `34032046904` | après quelques cycles du timer, puis campagne de deux heures si nécessaire |
> | Migration EF et rollout API/Worker | run `34032046904` réussi depuis `ead9e79` | terminé ; smoke API/public validé, heartbeat encore à relever |
> | `QT-08` — session de 48 h puis ouverture en mode avion (page jetable, `L0-12`) | | |
> | Propagation DNS des entrées ACS | `2026-09-02` | relevée le `2026-09-06`, CNAME DKIM complets alignés |
...
Le correctif du header administrateur Catalog est également limité au dépôt, à la branche
`fix/catalog-admin-header-link` et à la PR [#141](https://github.com/FlorianDrevet/Vole-Papillon-Damour/pull/141).
Aucun déploiement Catalog, compte, changement Entra ou donnée de bourse n'a été effectué ; le
contrôle sur l'URL publique reste à faire après le merge.
...
|---|---|---|---|
| `TXT` propriété + SPF + DKIM sur `mail` | `L0-8` | Oui — `TXT mail = "ms-domain-verification=57bdf09a-9c44-4816-b564-9a700cb19d07"`; `TXT mail = "v=spf1 include:spf.protection.outlook.com -all"`; CNAME `selector1-azurecomm-prod-net._domainkey.mail.volepapillondamour.fr` et `selector2-azurecomm-prod-net._domainkey.mail.volepapillondamour.fr` → sélecteurs correspondants `azurecomm.net` | `2026-09-06` |
...
| 2026-09-11 | Windows | **Catalog — accès administration depuis le badge.** Depuis `origin/main` dans le worktree `Vole-Papillon-Damour-admin-header-link`, suppression des liens `Espace administrateur` du header desktop et du drawer mobile, transformation du tag `Administration` en lien vers `/administration` et alignement à `42px` avec le bouton de compte. Validation : test rouge puis vert, 160 tests ChromeHeadless Catalog, build SSR/navigateur, `graphify update .` et `git diff --check` ; aucun déploiement ni changement hors dépôt. PR [#141](https://github.com/FlorianDrevet/Vole-Papillon-Damour/pull/141) ouverte. |
...
| 2026-09-06 | Windows | **Catalogue — ajustements UX.** Les fiches n'affichent plus le prix sur place et ne proposent l'alerte que lorsqu'aucun exemplaire n'est disponible. La recherche distingue clairement les livres dans la bourse de ceux qui n'y sont pas encore, permet de les ajouter à la liste de recherche, masque le fournisseur externe et tronque les métadonnées longues ; le déclencheur du compte administrateur n'affiche plus deux fois les initiales. Validation locale : 68 tests Catalog ; PR [#78](https://github.com/FlorianDrevet/Vole-Papillon-Damour/pull/78) ouverte. |
