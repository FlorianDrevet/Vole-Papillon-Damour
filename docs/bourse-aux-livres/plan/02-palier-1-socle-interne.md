...
**Critère de passage.** Après une bourse complète, l'écart entre le stock théorique et un
comptage physique par échantillon reste dans une marge acceptable, à définir avec
l'association. **C'est le test réel de la discipline de scan en caisse** — le principal
risque humain du projet, et il ne se teste que sur une vraie bourse.
...
🔧 Les quatre agrégats de `02` §1, leurs configurations EF Core, la migration 1 — `Books`,
`BookMovements`, `ScanSessions`, `BookAnnouncements`, `AssociationSettings`.
...
- **Collation insensible aux accents** sur `Title` et `Authors`, sans quoi la recherche est
  jugée inutilisable au premier essai.
...
✅ Tests de domaine, dans l'ordre de valeur de `03` §6 : `RG-15` (la table de priorité des
verdicts — peu de code, beaucoup de cas, une erreur invisible en production), puis `RG-10`
(le comptage doit inclure les annonces), puis `RG-01`.

## `P1-4` — Les handlers, et les trois qui comptent

🔧 Les tranches de `03` §2, **plus les sept oubliées** relevées en `revue.md` `R-15` :
...
🔧 La PWA de `04`, cette fois complète : IndexedDB à trois magasins, file de sortie
...
Les trois contraintes de `06` §2 sont des pièges réels : aucun handler ne dépend d'un
...
🔧 `DT-16`. Les mesures de `11` §4 et leurs règles d'alerte, en Bicep. **Plafond journalier
...
Ajouter le conteneur blob des couvertures, absent des quatre déclarés.

🔧 Pipelines : `scan-deploy.yml` et `worker-deploy.yml` sur le modèle existant. **L'API et
le worker se construisent et se déploient depuis le même commit** (`06` §2) — deux images,
...
plein texte, la requête de désengorgement de `05` §5, l'écriture d'un lot de scans en
...
## `P1-10` — La bourse d'épreuve

🧪 **Le vrai critère de passage, et il dure une bourse entière.**

Avant : prévenir les bénévoles que **le catalogue démarre vide** (`RG-48`). Un outil qui
répond « premier exemplaire » sur un livre qu'ils savent présent en cinq exemplaires perd
leur confiance si personne ne les a prévenus que le système apprend en marchant.

Pendant : trier, vendre, tout scanner.

Après : **un comptage physique par échantillon**, et comparer au stock théorique. L'écart
est l'indicateur de la discipline de scan en caisse — donc le principal indicateur de santé
du projet (`RG-34`, `05` §6).

📌 L'écart constaté, la taille de l'échantillon, la date. C'est la première valeur d'une
série qu'on suivra dans le temps.

**Si l'écart est hors marge**, le problème n'est pas technique : c'est que la caisse ne
scanne pas. Le corriger avant d'exposer un catalogue public au monde — publier un catalogue
sur un stock non fiable serait contre-productif.
