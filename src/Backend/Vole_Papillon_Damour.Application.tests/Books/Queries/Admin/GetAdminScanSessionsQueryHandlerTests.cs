...
public sealed class GetAdminScanSessionsQueryHandlerTests
{
    ...
    [Fact]
    public async Task Handle_WhenVolunteerNameIsMissing_DoesNotExposeVolunteerEmail()
    {
        await using var fixture = await ScanBookFixture.CreateAsync();
        var now = new DateTime(2026, 9, 5, 17, 0, 0, DateTimeKind.Utc);
        ...
        result.Value.Sessions[0].VolunteerName.Should().BeNull();
    }

    [Fact]
    public async Task Handle_WhenVolunteerHasName_ReturnsFirstAndLastName()
    {
        await using var fixture = await ScanBookFixture.CreateAsync();
        var now = new DateTime(2026, 9, 5, 17, 0, 0, DateTimeKind.Utc);
        var volunteerId = UserId.Create(Guid.Parse("00000000-0000-0000-0000-000000000003"));
        fixture.Context.Users.Add(User.CreateFromExternalIdentity(
            volunteerId,
            volunteerId.Value.ToString(),
            "volunteer@example.test",
        ...
        result.IsError.Should().BeFalse();
        result.Value.Sessions.Should().ContainSingle();
        result.Value.Sessions[0].VolunteerName.Should().Be("Ada Lovelace");
    }
}
