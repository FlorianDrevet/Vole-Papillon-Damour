...
describe('ApplicationInsightsErrorHandler', () => {
  ...
  it('wraps a non-Error value so the exception keeps a message', () => {
    ...
    new ApplicationInsightsErrorHandler(() => client).handleError('offline queue unavailable');

    const tracked = client.trackException.calls.mostRecent().args[0];
    expect(tracked.exception?.message).toBe('offline queue unavailable');
  });

  it('only logs when telemetry is disabled', () => {
