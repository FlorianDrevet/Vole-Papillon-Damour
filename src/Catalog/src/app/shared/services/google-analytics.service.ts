...
const GOOGLE_ANALYTICS_SCRIPT_ID = 'catalog-google-analytics-script';
...
export class GoogleAnalyticsService {
  ...
  get isConfigured(): boolean {
    return this.isBrowser && isConfigured(this.measurementId);
  }
  ...
  private setCollectionDisabled(analyticsWindow: GoogleAnalyticsWindow, disabled: boolean): void {
    analyticsWindow[`ga-disable-${this.measurementId}`] = disabled;
  }

  private get browserWindow(): GoogleAnalyticsWindow {
    return this.document.defaultView as unknown as GoogleAnalyticsWindow;
  }
}
