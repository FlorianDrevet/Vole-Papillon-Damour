...
describe('GoogleAnalyticsService', () => {
  let originalMeasurementId: string;
  let appendScript: jasmine.Spy;

  beforeEach(() => {
    originalMeasurementId = environment.googleAnalyticsMeasurementId;
    document.getElementById('catalog-google-analytics-script')?.remove();
  ...
  afterEach(() => {
    environment.googleAnalyticsMeasurementId = originalMeasurementId;
    document.getElementById('catalog-google-analytics-script')?.remove();
    const testWindow = window as GoogleAnalyticsTestWindow;
    delete testWindow.dataLayer;
    delete testWindow.gtag;
  ...
  it('enable_loads_the_tag_with_denied_defaults_and_granted_analytics', () => {
    environment.googleAnalyticsMeasurementId = 'G-TEST123';
    const service = createService();

    service.enable();

    const script = appendScript.calls.mostRecent().args[0] as HTMLScriptElement;
  ...
  it('enable_does_not_load_a_tag_when_the_measurement_id_is_not_configured', () => {
    environment.googleAnalyticsMeasurementId = '__GOOGLE_ANALYTICS_MEASUREMENT_ID__';
    const service = createService();
