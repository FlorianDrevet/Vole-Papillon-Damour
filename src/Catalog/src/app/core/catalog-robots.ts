import {catalogRoutePath, isCatalogAdministrationRoute, isKnownCatalogRoute} from './catalog-route';
...
export function catalogRobotsForUrl(url: string): CatalogRobotsDirective {
  ...
  return PRIVATE_ROUTE_PATHS.has(path) || isCatalogAdministrationRoute(path) || !isKnownCatalogRoute(path)
    ? 'noindex, nofollow'
    : 'index, follow';
}
