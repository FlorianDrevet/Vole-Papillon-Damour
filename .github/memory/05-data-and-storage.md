...
- The rare-book application slice provides create/update/publish/unpublish/delete/sell commands, photo upload/reorder/caption/delete commands, public/admin/cash queries, and contracts under `Application/RareBooks`, `Contracts/RareBooks`, and `Api/Controllers/RareBookController.cs`. Administration temporarily uses the existing `Administration` policy until the dedicated `LivresRares` role is introduced in lot 4; cash search uses `ScanVolunteer`.
